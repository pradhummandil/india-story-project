...
export const Route = createFileRoute("/api/rss/podcast")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          // Fetch custom episodes from PodcastEpisode table
          ...
          // Fallback to published stories with audio generation endpoints if episodes table is empty
...
<rss version="2.0" 
  ...
  <channel>
    ...
    ${itemsXml}
          ...
          console.error("Podcast RSS generation error:", err);
          return new Response("<error>Failed to generate podcast RSS</error>", { status: 500 });
        }
      },
    },
  },
