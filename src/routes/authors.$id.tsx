...
import { ArrowLeft, BookOpen, Compass, Mail, Globe, Sparkles, User, Clock, Eye, Play, Plus, Check, MapPin } from "lucide-react";
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="border border-border/80 bg-card p-6 md:p-10 hover:border-gold/30 transition-all duration-300 shadow-elegant mb-12">
            <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 md:gap-8">
              {author.avatar ? (
                <img src={author.avatar} alt={author.name} className="size-24 md:size-32 rounded-full object-cover border-2 border-gold/40 shadow-md shrink-0" />
              ) : (
                <div className="size-24 md:size-32 rounded-full bg-white/5 border-2 border-gold/40 flex items-center justify-center text-3xl font-bold text-gold uppercase shrink-0">{author.name[0]}</div>
              )}
              <div className="flex-1 space-y-4">
                <div>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-center md:justify-start">
                    <h1 className="font-display text-3xl md:text-4xl font-bold leading-tight">{author.name}</h1>
                    <span className="bg-gold/10 border border-gold/35 text-gold text-[9px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-none font-sans self-center">Verified Chronicler</span>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1 font-sans flex items-center justify-center md:justify-start gap-1">
                    <MapPin className="size-3.5 text-gold" /> {author.location} • Joined {new Date(author.joinedAt).toLocaleDateString()}
                  </p>
                </div>
                <p className="text-sm md:text-base leading-relaxed text-muted-foreground/95 max-w-2xl font-sans font-medium">
                  {author.bio || "Writer and visual journalist capturing deep stories of resilience, culture, and progress from the heartlands of India."}
                </p>
            ...
            <div className="mt-10 pt-8 border-t border-border/40 grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-xs uppercase font-bold tracking-wider text-muted-foreground font-sans">
              <div className="border-r border-border/40 last:border-0">
                <span className="text-white text-xl md:text-2xl block font-mono font-medium">{author.storyCount}</span>
                <span className="text-[10px] font-sans">Stories</span>
              </div>
              <div className="border-r border-border/40 last:border-0">
                <span className="text-white text-xl md:text-2xl block font-mono font-medium">{author.videoCount}</span>
                <span className="text-[10px] font-sans">Videos</span>
              </div>
              <div className="border-r border-border/40 last:border-0 font-mono">
                <span className="text-white text-xl md:text-2xl block font-medium">{author.followersCount}</span>
              ...
              <div>
                <span className="text-white text-xl md:text-2xl block font-mono font-medium">
                  {author.totalViews > 1000 ? `${(author.totalViews / 1000).toFixed(1)}k` : author.totalViews}
                </span>
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed mt-1">{story.excerpt}</p>
                      </div>
                      <div className="mt-4 pt-3 border-t border-border/30 flex items-center justify-between text-[10px] text-muted-foreground font-sans">
                        ...
                        <span className="flex items-center gap-1"><Eye className="size-3" /> {story.viewCount} reads</span>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {videos.map((video) => (
                  ...
                  >
                    <div className="aspect-video relative overflow-hidden bg-black">
                      <img src={video.thumbnail} alt={video.title} className="absolute inset-0 w-full h-full object-cover filter brightness-90 group-hover:scale-105 transition-transform duration-500" />
                      <Link to="/videos/$slug" params={{ slug: video.slug }} className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/40 transition-colors">
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed mt-1">{video.excerpt}</p>
                      </div>
                      <div className="mt-4 pt-3 border-t border-border/30 flex items-center justify-between text-[10px] text-muted-foreground font-sans">
