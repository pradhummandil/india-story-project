...
export function YouMayAlsoLike() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="mb-8">
          <p className="text-xs uppercase tracking-widest text-gold mb-3">Because you opened a story</p>
          <h2 className="font-display text-3xl md:text-4xl">You May Also Like</h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {similar.map((s, i) => <StoryCard key={s.id} story={s} index={i} />)}
        </div>
        ...
        {continueExploring.length > 0 && (
          <div className="mt-16">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {continueExploring.map((s, i) => <StoryCard key={s.id} story={s} index={i} />)}
            </div>
          </div>
        )}
      </motion.section>
